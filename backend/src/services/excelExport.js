...
export const exportApplicationsToExcel = async (req, res) => {
    ...
    try {
        ...
        const reformAreasRaw = await prisma.reformArea.findMany();
        const reformAreas = reformAreasRaw.map(r => r.data).filter(r => !editionId || r.editionId === editionId);

        const usersRaw = await prisma.user.findMany();
        const users = usersRaw.map(u => u); // Flat table structure, no parsing needed

        const formFieldsRaw = await prisma.formField.findMany();
        let formFields = formFieldsRaw.map(f => f.data);
        if (editionId) {
            formFields = formFields.filter(f => f.editionId === editionId);
        }
        formFields.sort((a, b) => {
        ...
        let applications = applicationsRaw.map(a => a.data);
        applications = applications.filter(a => a.status !== 'Draft');
        if (editionId) {
            applications = applications.filter(a => a.editionId === editionId);
        }
        ...
        for (const app of applications) {
            ...
            const answersRaw = await prisma.applicationAnswer.findMany();
            const answers = answersRaw.map(a => a.data).filter(a => a.applicationId === app.id);
