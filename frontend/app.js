...
function pushToNavHistory(state) {
...
function sendPasswordOtp(email, otp, subject, userId) {
      ...
      allEditions.forEach(ed => {
        if (ed.name.toLowerCase().includes(query) || (ed.description && ed.description.toLowerCase().includes(query))) {
          if (!isAdminOrSuper) {
            ...
            if (isAssigned && ed.status === 'published') {
...
function openEditionTracker(editionId) {
...
function renderSchemaEditorAdmin(container) {
...
function openEditUserModal(userId, container) {
  const user = getUserById(userId);
  if (!user) return;
...
// ─── GUIDELINES PANEL ─────────────────────────────────────────────────────
function renderGuidelinesPanel(container) {
  const editions = getEditions();
...
function openAddGuidelineModal(editionId, container) {
  const backdrop = document.createElement('div');
  backdrop.className = 'modal-backdrop-custom visible';
  backdrop.innerHTML = `
...
function exportAllSubmissionsExcelFunc() {
    const db = getDb();
...
function openExportEditionModal(onConfirm) {
  const editions = getEditions();
...
function renderUserSidebar() {
  ...
  const tabs = [
    ...
    { id: 'drafts',       label: 'Draft Applications', icon: '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4z"/>', badge: draftCount > 0 ? String(draftCount) : '' },
    { id: 'submitted',    label: 'In Review',          icon: '<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>', badge: '' },
...
function switchUserTab(tab) {
  ...
  switch (tab) {
    ...
    case 'submitted':   renderUserAppsFiltered(mainView, 'Submitted'); break;
...
function renderUserDashboard(container) {
  ...
  container.querySelector('#btn-new-app-dash')?.addEventListener('click', () => switchUserTab('apply'));
  container.querySelector('#btn-start-first-app')?.addEventListener('click', () => switchUserTab('apply'));

  container.querySelectorAll('.kpi-card-clickable').forEach(card => {
    card.addEventListener('click', () => switchUserTab(card.dataset.tab));
  });
...
function startNewApplication(editionId, container) {
  const user = getCurrentUser();
  ...
  openApplicationForm(app.id, container);
}

function renderAssignedEditionsPage(container) {
  const user = getCurrentUser();
  const rawUserAssignments = getAssignments ? getAssignments(user.id) : [];
  const userAssignments = rawUserAssignments.filter(isAssignmentValid);
  ...
  const inProgressEditions = [];
  const underReviewEditions = [];
  ...
  function createProgressRingHtml(percentage, size = 52, strokeWidth = 5, color = 'var(--accent-indigo)') {
    ...
    return `
      <div class="progress-ring-container" style="width: ${size}px; height: ${size}px; position: relative; display: inline-flex; align-items: center; justify-content: center;">
        <svg width="${size}" height="${size}" style="transform: rotate(-90deg);">
          ...
          <circle class="progress-ring-circle" stroke="${color}" stroke-width="${strokeWidth}" fill="transparent" r="${radius}" cx="${size/2}" cy="${size/2}"
                  stroke-dasharray="${circumference} ${circumference}" stroke-dashoffset="${offset}" style="transition: stroke-dashoffset 0.35s;"/>
        </svg>
        <span class="progress-ring-text" style="position: absolute; font-size: 11px; font-weight: 700; color: var(--text-main);">${percentage}%</span>
      </div>
    `;
  }
  ...
  assignedEditionIds.forEach(eid => {
    ...
    if (app) {
      ...
      } else if (status === 'Additional Documents Requested') {
        actionLabel = 'Upload Docs';
        actionCls = 'btn-primary';
      } else {
        actionLabel = 'View Workspace';
        actionCls = 'btn-outline';
      }
      
      const score = calculateApplicationScore(app.id);
      const maxScore = calculateApplicationMaxScore(app.id) || 1;
      const pct = ((score / maxScore) * 100).toFixed(0);
      const approvedCount = getAnswersByApplication(app.id).filter(a => a.questionStatus === 'Approved').length;
      const hasAnyScore = score > 0 || approvedCount > 0;
      const hasBeenReviewed = ['Approved', 'Rejected', 'Additional Documents Requested'].includes(status) || hasAnyScore;
    ...
    const dueDate = ed.endDate 
      ? new Date(ed.endDate).toLocaleDateString('en-IN') 
      : 'N/A';
    ...
    } else if (status === 'Draft' || status === 'Additional Documents Requested') {
      inProgressEditions.push(cardData);
    } else if (status === 'Approved' || status === 'Rejected') {
      completedEditions.push(cardData);
    } else {
      underReviewEditions.push(cardData);
    }
  });
  ...
  if (curTab === 'active') {
    ...
    emptyMsg = 'No active assignments. All assigned nodal offices have started application drafts.';
  } else if (curTab === 'inprogress') {
    tabCards = inProgressEditions;
    emptyMsg = 'No compliance submissions are currently in progress.';
  } else if (curTab === 'underreview') {
    tabCards = underReviewEditions;
    emptyMsg = 'No compliance submissions are under review.';
  } else {
    tabCards = completedEditions;
    emptyMsg = 'No completed compliance submissions yet.';
  }

  const renderCard = (c) => {
    const statusCls = _statusClass(c.status);
    const appAction = c.status === 'Not Started' 
      ? `data-action="start" data-edition-id="${c.ed.id}"`
      : `data-action="open" data-app-id="${c.app.id}"`;
      
    let colorCls = 'blue';
    if (c.status === 'Draft') colorCls = 'orange';
    else if (c.status === 'Additional Documents Requested') colorCls = 'yellow';
    else if (['Submitted', 'Resubmitted', 'Under Review'].includes(c.status)) colorCls = 'purple';
    else if (c.status === 'Approved') colorCls = 'green';
    else if (c.status === 'Rejected') colorCls = 'red';

    let progressPct = 0;
    if (c.app) {
      const score = calculateApplicationScore(c.app.id) || 0;
      const maxScore = calculateApplicationMaxScore(c.app.id) || 1;
      progressPct = Math.round((score / maxScore) * 100);
    }
    ...
    let countdownCls = '';
    if (c.ed.endDate) {
      const diff = new Date(c.ed.endDate) - new Date();
      const days = Math.ceil(diff / (1000 * 60 * 60 * 24));
      if (days > 0) {
        countdownText = `⏳ ${days} days left`;
      } else if (days === 0) {
        countdownText = `⏰ Due today`;
        countdownCls = 'text-warning';
      } else {
        countdownText = `🛑 Overdue by ${Math.abs(days)} days`;
        countdownCls = 'text-danger';
      }
    }

    // Compliance Health Badge label:
    let healthLabel = 'No Data';
    let healthClass = 'health-no-data';
    if (c.app) {
      if (progressPct >= 80) {
        ...
        healthClass = 'health-needs-imp';
      } else {
        healthLabel = 'Critical Health';
        healthClass = 'health-critical';
      }
    }

    return `
      <div class="assigned-edition-card glass-card glass-card-enhanced status-border-${colorCls}" style="padding:20px; display:flex; flex-direction:column; justify-content:space-between; position:relative;">
        <div>
          <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom: 12px;">
            <div style="display:flex; align-items:center; gap:8px;">
              <div class="assigned-edition-icon-container" style="background:rgba(99,102,241,0.06); color:var(--primary); padding:6px; border-radius:8px;">
              ...
              <div>
                <h3 class="assigned-edition-title" style="font-size:14px; font-weight:700; margin:0; line-height:1.35; color:var(--text-main);">${c.ed.name}</h3>
                <span style="font-size:11px; color:var(--text-muted);">Duration: ${c.ed.duration || c.ed.version || 'Annual'}</span>
              </div>
            </div>
            <span class="status-badge ${statusCls}" style="font-size:9.5px; font-weight:700; padding:2px 8px; border-radius:99px; text-transform:uppercase;">${c.status}</span>
          </div>

          <p class="assigned-edition-desc" style="margin-bottom:14px; font-size:12.5px; color:var(--text-muted); height:36px; overflow:hidden; text-overflow:ellipsis; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;">${c.ed.description || 'No description available.'}</p>
          
          <div style="display: flex; align-items: center; gap: 14px; margin-bottom: 16px;">
            ${createProgressRingHtml(progressPct, 48, 4.5, `var(--accent-${colorCls})`)}
            <div style="display:flex; flex-direction:column; gap:2px;">
              <span class="status-badge ${healthClass}" style="font-size:9px; font-weight:700; padding:1px 6px; border-radius:4px; text-transform:uppercase; align-self: flex-start;">${healthLabel}</span>
              <span style="font-size:11px; color:var(--text-muted);">Score: <strong>${c.scoreText}</strong></span>
            </div>
          </div>
        </div>

        <div style="border-top:1px dashed var(--border-color); padding-top:12px; margin-top:auto;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; font-size:11.5px; color:var(--text-muted);">
            <span>Assigned: <strong>${c.assignedDate}</strong></span>
            <span class="${countdownCls}"><strong>${countdownText}</strong></span>
          </div>
          <button class="btn ${c.actionCls} btn-sm btn-assigned-action" style="width:100%; font-weight:600;" ${appAction}>
            ${c.actionLabel}
          </button>
        </div>
      </div>
  ...
  container.innerHTML = `
    <div class="section-card" style="margin-bottom:24px;">
      ...
      <p style="color:var(--text-muted);font-size:14px;">All compliance editions assigned to your department. Start or manage submissions below.</p>
    </div>
    ...
    <div class="workspace-nav-tabs" style="margin-bottom:20px; display:flex; gap:8px; border-bottom: 1px solid var(--border-color); padding-bottom:8px; flex-wrap:wrap;">
      <button class="workspace-tab-link ${curTab === 'active' ? 'active' : ''}" data-tab="active" style="background:none; border:none; padding:8px 16px; cursor:pointer; font-size:13px; font-weight:600; color:${curTab === 'active' ? 'var(--primary)' : 'var(--text-muted)'}; border-bottom:${curTab === 'active' ? '2px solid var(--primary)' : 'none'};">
        Active Assignments (${activeEditions.length})
      </button>
      <button class="workspace-tab-link ${curTab === 'inprogress' ? 'active' : ''}" data-tab="inprogress" style="background:none; border:none; padding:8px 16px; cursor:pointer; font-size:13px; font-weight:600; color:${curTab === 'inprogress' ? 'var(--primary)' : 'var(--text-muted)'}; border-bottom:${curTab === 'inprogress' ? '2px solid var(--primary)' : 'none'};">
        In Progress (${inProgressEditions.length})
      </button>
      <button class="workspace-tab-link ${curTab === 'underreview' ? 'active' : ''}" data-tab="underreview" style="background:none; border:none; padding:8px 16px; cursor:pointer; font-size:13px; font-weight:600; color:${curTab === 'underreview' ? 'var(--primary)' : 'var(--text-muted)'}; border-bottom:${curTab === 'underreview' ? '2px solid var(--primary)' : 'none'};">
        Under Review (${underReviewEditions.length})
      </button>
      <button class="workspace-tab-link ${curTab === 'completed' ? 'active' : ''}" data-tab="completed" style="background:none; border:none; padding:8px 16px; cursor:pointer; font-size:13px; font-weight:600; color:${curTab === 'completed' ? 'var(--primary)' : 'var(--text-muted)'}; border-bottom:${curTab === 'completed' ? '2px solid var(--primary)' : 'none'};">
        Completed (${completedEditions.length})
      </button>
    </div>

    <div id="assigned-editions-list-content">
      ${tabCards.length === 0 ? `
        <div style="padding: 40px; background: rgba(226,232,240,0.1); border-radius:12px; border:1px dashed var(--border-color); text-align:center; color:var(--text-muted); font-size:14px;">
          ${emptyMsg}
        </div>
      ` : `
        <div class="assigned-editions-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(310px, 1fr)); gap: 20px;">
          ${tabCards.map(renderCard).join('')}
        </div>
      `}
    </div>
  `;
  ...
  container.querySelectorAll('.btn-assigned-action').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (action === 'start') {
        ...
        if (app.status === 'Draft' || app.status === 'Additional Documents Requested') {
          openApplicationForm(appId, container);
        } else {
          openApplicationDetail(appId, container);
        }
      }
    });
  });
...
function isSectionAssignedToUser(sec, user) {
  if (user.role === 'admin' || user.role === 'superadmin') return true;
  return isSectionAssignedToUserStore(sec, user.id);
}

function isFieldAssignedToUser(f, user) {
  if (user.role === 'admin' || user.role === 'superadmin') return true;
  return isFieldAssignedToUserStore(f, user.id);
}

function getGuidelinePageForQuestion(num) {
  if (!num) return null;
  ...
  container.innerHTML = `
    <div style="margin-bottom:16px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
      ...
      <div class="breadcrumbs-container" style="font-size:12px;color:var(--text-muted);">
        <span style="color:var(--primary);font-weight:500;">My Portal</span> /
        <span style="color:var(--primary);font-weight:500;">${edition?.name}</span> /
      ...
      ${isFrozen ? `<span class="status-badge status-submitted" style="margin-left:auto;">${app.status}</span>` : `
        <div style="margin-left:auto;display:flex;gap:8px;">
          <button class="btn btn-secondary btn-sm" id="btn-save-draft">💾 Save Draft</button>
    ...
    ${!isFrozen ? `
      <div style="display:flex;gap:12px;margin-top:24px;justify-content:flex-end;">
        <button class="btn btn-secondary" id="btn-save-draft-bottom">💾 Save Draft</button>
  ...
  const doSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Redirect user tab to "Submitted Applications" page which is 'submitted'
        activeUserTab = 'submitted';
        renderUserSidebar();
        switchUserTab('submitted');
...
function renderUserElementInput(el, fieldId, value, disabled, appId) {
...
function renderFieldInput(field, value, disabled, appId) {
  const dis = disabled ? 'disabled' : '';
...
function renderUserAppsFiltered(container, status) {
  const user = getCurrentUser();
...
function openApplicationDetail(appId, container) {
  const user = getCurrentUser();
  const app = getApplicationById(appId);
  if (!app) return;
  if (user.role === 'user' && app.userId !== user.id) {
...
function showNotificationsDropdown(event) {
  const user = getCurrentUser();
  if (!user) return;
  if (isAdmin() || isSuperAdmin()) {
...
function renderUserProfilePage(container) {
  const user = getCurrentUser();
  const isStateUser = user.role === 'user';
...
// ─── PUBLISH WIZARD ────────────────────────────────────────────────────────
function renderPublisherPanel(container) {
  if (!container) return;
  const editions = getEditions();
...
function openReassignModal(assignmentId, container) {
...
// ═══════════════════════════════════════════════════════════════════════════
function renderDepartmentsPanel(container) {
  const depts = getDepartments();
...
function openCreateDepartmentModal(container) {
  const backdrop = document.createElement('div');
  backdrop.className = 'modal-backdrop-custom visible';
  backdrop.innerHTML = `
    <div class="modal-card-custom animate-modal-in" style="max-width:500px; text-align:left;">
...
function openEditDepartmentModal(deptId, container) {
  const depts = getDepartments();
