...
import {
  DB_KEY, MIGRATION_KEY, DB_VERSION, createEmptyDb, buildDefaultUsers,
  DEFAULT_SRF_6_EDITION, NOTIFICATION_EVENTS, QUESTION_STATUS, DOC_STATUS
} from './schema.js';
...
export async function initStore() {
  try {
    ...
    if (res.ok) {
      ...
      console.warn('[Store] Failed to load database from API, trying localStorage fallback.');
      _loadFromLocalStorage();
    }
  } catch (error) {
    console.warn('[Store] Network error loading database from API, trying localStorage fallback:', error);
    _loadFromLocalStorage();
  }
  ...
  if (_db) {
    ...
    if (_db.editions) {
      ...
      _db.editions = _db.editions.filter((e, idx, self) => self.findIndex(x => x.id === e.id) === idx);
      if (_db.editions.length !== origCount) changed = true;
    }
    if (_db.reformAreas) {
      const origCount = _db.reformAreas.length;
      _db.reformAreas = _db.reformAreas.filter((r, idx, self) => 
        self.findIndex(x => x.editionId === r.editionId && x.name === r.name) === idx
      );
    ...
    if (_db.formFields) {
      ...
      _db.formFields = _db.formFields.filter((f, idx, self) => {
        if (f.num) {
          return self.findIndex(x => x.editionId === f.editionId && x.num === f.num) === idx;
        }
    ...
    if (_db.users) {
      ...
      _db.users = _db.users.filter((u, idx, self) => self.findIndex(x => x.id === u.id || x.username === u.username) === idx);
            ...
            _db.formFields.forEach(field => {
              if (field.assignment) {
                if (field.assignment.userIds) {
                  field.assignment.userIds = field.assignment.userIds.map(uid => uid === oldUsername ? user.username : uid);
                }
                if (field.assignment.users) {
                  field.assignment.users = field.assignment.users.map(uid => uid === oldUsername ? user.username : uid);
                }
              }
            });
          }
    ...
    if (_db.assignments) {
      ...
      const apIds = new Set((_db.formFields || []).filter(f => f.actionPointId).map(f => f.actionPointId));
      ...
      _db.assignments.forEach(a => {
        const isDup = uniqueAssignments.some(x => 
          x.userId === a.userId &&
          x.editionId === a.editionId &&
          x.type === a.type &&
          (a.type === 'Reform Area' ? (x.sectionId === a.sectionId || x.reformAreaId === a.reformAreaId) : true) &&
          (a.type === 'Action Point' ? x.actionPointId === a.actionPointId : true) &&
          (a.type === 'Question' ? (x.questionId === a.questionId || x.fieldId === a.fieldId) : true)
        );
        if (!isDup) {
    ...
    if (_db.applications && _db.applications.length > 0) {
      ...
      Object.keys(groups).forEach(key => {
        ...
        const approvedOrRejected = apps.filter(a => a.status === 'Approved' || a.status === 'Rejected');
      ...
      if (appChanged) {
        console.log(`[Store] Deduplicated applications. Removed ${deletedAppIds.length} duplicate applications.`);
        _db.applications = keptApps;
        if (_db.applicationAnswers) {
          _db.applicationAnswers = _db.applicationAnswers.filter(ans => !deletedAppIds.includes(ans.applicationId));
        }
        changed = true;
      }
    }
    if (_db.applicationAnswers) {
      ...
      _db.applicationAnswers.forEach(ans => {
        const firstIdx = uniqueAnswers.findIndex(x => x.applicationId === ans.applicationId && x.fieldId === ans.fieldId);
        if (firstIdx === -1) {
          uniqueAnswers.push(ans);
        } else {
          const existing = uniqueAnswers[firstIdx];
          const statusPriority = { 'Approved': 4, 'Rejected': 3, 'Submitted': 2, 'Draft': 1 };
          const pExisting = statusPriority[existing.questionStatus] || 0;
          const pCurrent = statusPriority[ans.questionStatus] || 0;
          if (pCurrent > pExisting || (pCurrent === pExisting && new Date(ans.updatedAt || 0) > new Date(existing.updatedAt || 0))) {
...
function _loadFromLocalStorage() {
  ...
  if (raw) {
    try { _db = JSON.parse(raw); } catch { _db = null; }
  }
...
function _migrateFromV2() {
  ...
  try {
    ...
    if (v2.reformAreas?.length) _db.reformAreas = v2.reformAreas.map(r => ({...r}));
    else if (v2.sections?.length) _db.reformAreas = v2.sections.map(s => ({...s, id: s.id}));
    // Migrate form fields
    if (v2.formFields?.length) _db.formFields = v2.formFields.map(f => ({...f}));
    // Migrate applications
    if (v2.applications?.length) _db.applications = v2.applications.map(a => ({...a}));
    // Migrate answers with new fields
    if (v2.applicationAnswers?.length) {
      _db.applicationAnswers = v2.applicationAnswers.map(a => ({
        ...a,
        questionStatus: a.questionStatus || (a.status === 'Submitted' ? QUESTION_STATUS.SUBMITTED : QUESTION_STATUS.DRAFT),
  ...
  } catch (e) { console.warn('[SRF v3] Migration error:', e); }
}
...
function _ensureDefaultEdition() {
  ...
  const existingRAs = (_db.reformAreas || []).filter(r => r.editionId === DEFAULT_SRF_6_EDITION.id);
  const existingFields = (_db.formFields || []).filter(f => f.editionId === DEFAULT_SRF_6_EDITION.id);
  const firstField = existingFields.find(f => f.num === '1.1');
  const needsReseed = !firstField || 
                      firstField.actionPointTitle?.startsWith('Action Point') || 
                      existingFields.some(f => f.num && (f.fieldType !== 'radio' || !f.options || f.options.length !== 2 || f.options[0] !== 'Yes' || f.options[1] !== 'No')) ||
                      existingFields.some(f => f.num && f.num !== '1.1' && f.docs?.some(d => d.name === 'Upload Supporting Document'));

  if (existingRAs.length < 7 || existingFields.length < 49 || needsReseed) {
    console.log('[Store] Seeding default SRF 6.0 schema with official Action Point titles...');
    _db.reformAreas = (_db.reformAreas || []).filter(r => r.editionId !== DEFAULT_SRF_6_EDITION.id);
    _db.formFields = (_db.formFields || []).filter(f => f.editionId !== DEFAULT_SRF_6_EDITION.id);
  ...
  const field11 = (_db.formFields || []).find(f => f.num === '1.1' && f.editionId === DEFAULT_SRF_6_EDITION.id);
  if (field11) {
    const hasCorrectDocs = field11.docs?.length === 2 && 
      (field11.docs[0].name.startsWith("Government Order") || field11.docs[0].name.startsWith("Date of official")) && 
      field11.docs[1].name.startsWith("G.O.");
      
    const isCorrectRadio = field11.fieldType === 'radio' && 
    ...
    if (!hasCorrectDocs || !isCorrectRadio) {
      console.log('[Store] Force resetting Question 1.1 properties to match Question 1.2 radio layout...');
      field11.fieldType = 'radio';
      field11.label = 'Question 1.1: Does your State/UT have an active Startup Policy?';
      field11.text = 'Question 1.1: Does your State/UT have an active Startup Policy?';
      ...
      field11.elements = [
        {
          id: 'el_srf6_1_1_1',
          type: 'radio',
          label: 'Question 1.1: Does your State/UT have an active Startup Policy?',
          required: true,
          options: ['Yes', 'No']
        }
      ];
      field11.docs = [
        { id: 'doc_srf6_1_1_1', name: "Date of official implementation of the State/UT Startup Policy, as per the Government Order or Notification. This should indicate when the policy came into effect and began guiding startup- related initiatives in the State/UT.", requirement: 'optional' },
        { id: 'doc_srf6_1_1_2', name: "G.O. / Notification and Policy Document for State / UT Startup Policy", requirement: 'optional' }
      ];
      _save();
    }
  }
  ...
  (_db.applications || []).forEach(app => {
    if (app.status === 'Submitted') {
      const lastTimelineEntry = app.timeline && app.timeline[app.timeline.length - 1];
      if (lastTimelineEntry && lastTimelineEntry.action.startsWith('Question submitted')) {
        console.log(`[Store] Resetting application ${app.id} status to 'Draft' because it was locked by a question submit.`);
        app.status = 'Draft';
        _save();
      }
    }
  ...
  try {
    if (_db) {
      ...
      try {
        ...
        console.error('[Store] Network error saving database to MongoDB API:', e);
      }
      ...
      try {
        ...
        try { localStorage.setItem(DB_KEY, JSON.stringify(_db)); } catch(err) {}
      }
...
export async function forceSave() { await _save(); }
export function getDb() { return _db; }
export function getSettings() { return _db.settings || {}; }
export function updateSettings(u) { Object.assign(_db.settings, u); _save(); }
...
export function getEditions(includeDeleted = false) { 
  ...
  try {
    ...
    if (sessionRaw) {
      ...
      if (user && (user.role === 'superadmin' || user.role === 'admin' || user.role === 'reviewer')) {
        return active;
      }
    }
  } catch (e) {}
...
export function getEditionById(id) { return (_db.editions || []).find(e => e.id === id); }
...
export function deleteEdition(id) {
  ...
  if (edition) {
    ...
    const reformAreas = (_db.reformAreas || []).filter(r => r.editionId === id);
    ...
    _db.applications = (_db.applications || []).filter(a => a.editionId !== id);
    const appIdsToDelete = apps.map(a => a.id);
    _db.applicationAnswers = (_db.applicationAnswers || []).filter(a => !appIdsToDelete.includes(a.applicationId));
    ...
    _db.documentRules = (_db.documentRules || []).filter(r => r.editionId !== id);
    _db.schemaVersions = (_db.schemaVersions || []).filter(v => v.editionId !== id);
    _db.reassignmentHistory = (_db.reassignmentHistory || []).filter(h => h.editionId !== id);

    addAuditLog(currentUserId, `Deleted edition: ${edition.name}`, 'edition', id);
    _save();
  }
}
...
export function getReformAreaById(id) { return (_db.reformAreas||[]).find(r => r.id === id); }
...
export function createReformArea(editionId, data) {
  ...
  const ra = {
    ...
    color: data.color || ['#4f46e5','#0284c7','#7e22ce','#10b981','#d97706','#ef4444','#0891b2','#db2777'][existing.length % 8],
    marks: data.marks || 10
  };
...
export function updateReformArea(id, data) {
  ...
  if (idx !== -1) { Object.assign(_db.reformAreas[idx], data, { id }); _save(); }
...
export function createField(editionId, reformAreaId, data) {
  ...
  const field = {
    ...
    orderIndex: data.orderIndex ?? existing.length,
    isLayoutElement: ['heading','subheading','description','instruction','divider','card','banner','notes','warning','image','hyperlink'].includes(data.fieldType),
...
export function updateField(id, data) {
  ...
  if (idx !== -1) { Object.assign(_db.formFields[idx], data, { id }); _save(); }
...
export function saveSchemaVersion(editionId, userId, note = '') {
  ...
  const versions = (_db.schemaVersions || []).filter(v => v.editionId === editionId);
  ...
  if (all.length > 20) {
    ...
    _db.schemaVersions = _db.schemaVersions.filter(v => !toRemove.includes(v.id));
  }
  _save();
  return sv;
}
...
export function getSchemaVersions(editionId) {
  return (_db.schemaVersions || []).filter(v => v.editionId === editionId).sort((a,b) => b.version - a.version);
}
...
export function restoreSchemaVersion(versionId, userId) {
  ...
  addAuditLog(userId, `Restored schema to ${sv.versionLabel}`, 'schema', sv.editionId);
  _save();
  return true;
}
...
export function getApplications(filters = {}) {
  ...
  let apps = (_db.applications || []).filter(a => activeEditionIds.includes(a.editionId));
  ...
  try {
    ...
    if (sessionRaw) {
      ...
      } else if (user && (user.role === 'admin' || user.role === 'reviewer' || user.role === 'superadmin')) {
  ...
  if (filters.editionId)  apps = apps.filter(a => a.editionId === filters.editionId);
  ...
  if (filters.search) {
    ...
    apps = apps.filter(a => {
      ...
      return a.id.toLowerCase().startsWith(q) ||
  ...
  if (filters.sortBy) {
    ...
    apps = [...apps].sort((a,b) => new Date(b.updatedAt||0) - new Date(a.updatedAt||0));
  }
  const total = apps.length;
  const page = filters.page || 1;
  const pageSize = filters.pageSize || 25;
  return { items: apps.slice((page-1)*pageSize, page*pageSize), total, page, pageSize, totalPages: Math.ceil(total/pageSize) };
}

export function getApplicationById(id) { return (_db.applications||[]).find(a => a.id === id); }
export function getApplicationsByUser(userId) {
  const user = getUserById(userId);
  if (!user) return [];
  const activeEditionIds = getEditions(false).filter(e => e.status === 'published').map(e => e.id);
  return (_db.applications || []).filter(a => a.userId === userId && activeEditionIds.includes(a.editionId));
}
...
export function createApplication(userId, editionId, category, duration) {
  const user = getUserById(userId);
  const existing = _db.applications.find(a => a.userId === userId && a.editionId === editionId && a.status !== 'Rejected');
  ...
  const app = {
    ...
    additionalDocsNote: '',
    reviewerComments: '',
    timeline: [{ action: 'Application created', timestamp: new Date().toISOString(), by: userId }],
    comments: [],
    reformAreaStatuses: {},  // { [reformAreaId]: 'Draft'|'Submitted'|'Approved' }
    statusHistory: [{ status: 'Draft', timestamp: new Date().toISOString(), by: userId }],
    submissions: []
  };
  ...
  if (idx !== -1) {
    ...
    if (newStatus && newStatus !== oldStatus) {
      ...
      const allowedTransitions = {
        ...
        'Submitted': ['Under Review', 'Approved', 'Rejected', 'Additional Documents Requested'],
        'Resubmitted': ['Under Review', 'Approved', 'Rejected', 'Additional Documents Requested'],
        'Under Review': ['Approved', 'Rejected', 'Additional Documents Requested'],
        'Rejected': ['Draft'],
        'Approved': ['Draft'],
      ...
      if (currentUserRole !== 'superadmin') {
        ...
        if (!allowed.includes(newStatus)) {
          console.warn(`[Status Flow Block] Blocked invalid status change from ${oldStatus} to ${newStatus} for app ${id}`);
          delete data.status;
        }
      }
...
export function deleteApplication(id) {
  ...
  if (app) {
    ...
    const answers = (_db.applicationAnswers || []).filter(a => a.applicationId === id);
    ...
    _db.recycleBin.push({
      id: 'rb_app_' + Date.now() + '_' + Math.random().toString(36).substr(2, 4),
      type: 'application',
      appId: id,
  ...
  _db.applicationAnswers = _db.applicationAnswers.filter(a => a.applicationId !== id);
  _save();
}
...
export function isQuestionFilled(ans, field) {
  if (!ans) return false;
  const isUploadType = field && (['file', 'pdf', 'imageupload'].includes(field.fieldType) || field.isUploadElement);
  if (isUploadType) {
  ...
  if (valStr === '' || valStr === 'not answered' || valStr === 'not answered yet') return false;
...
export function isAnswerValueValid(value, field) {
  ...
  if (valStr === '' || valStr === 'no' || valStr === 'not answered' ||
      valStr === 'not answered yet' || valStr === 'not applicable' || valStr === 'n/a') {
    return false;
  }
  return true;
}
...
export function approveApplication(appId, adminId, comments = '') {
  ...
  addTimelineEntry(appId, comments ? `Approved: ${comments}` : 'Application approved', adminId);
  addAuditLog(adminId, comments ? `Approved application: ${comments}` : `Approved application: ${appId}`, 'application', appId);
  ...
  if (app) {
    ...
    fields.forEach(field => {
      ...
      if (ans.questionStatus !== QUESTION_STATUS.APPROVED && ans.questionStatus !== QUESTION_STATUS.REJECTED) {
        ...
        ans.questionScore  = isAnswered ? (field.maxScore || field.weight || 1) : 0;
...
export function rejectApplication(appId, adminId, reason) {
  updateApplication(appId, { status: 'Rejected', rejectionReason: reason, reviewerComments: reason });
  addTimelineEntry(appId, `Rejected: ${reason}`, adminId);
  addAuditLog(adminId, `Application rejected: ${appId} - Reason: ${reason}`, 'application', appId);
}

export function requestAdditionalDocs(appId, adminId, note) {
  updateApplication(appId, { status: 'Additional Documents Requested', additionalDocsNote: note });
...
export function isFieldAssignedToUser(f, userId) {
  ...
  const editionAssignments = (_db.assignments || []).filter(a => a.userId === userId && a.editionId === f.editionId);
  ...
  const parentRA = (_db.reformAreas || []).find(s => s.id === f.reformAreaId && s.editionId === f.editionId);
...
export function isSectionAssignedToUser(sec, userId) {
  ...
  const raAssignments = (_db.assignments || []).filter(a => a.userId === userId && a.editionId === sec.editionId);
  const isRAAssigned = raAssignments.some(a => {
    if ((!a.type || a.type === 'Reform Area') && (a.sectionId === sec.id || a.reformAreaId === sec.id)) return true;
    return false;
  });
...
export function submitReformArea(appId, reformAreaId, userId) {
  ...
  const fields = getFieldsByEdition(app.editionId).filter(f => f.reformAreaId === reformAreaId && isFieldAssignedToUser(f, userId));
  const answers = getAnswersByApplication(appId);
  const answersMap = {};
    ...
    if (f.mandatory && !f.isLayoutElement) {
      ...
      if (f.docs && f.docs.length > 0) {
        ...
        for (const doc of mandatoryDocs) {
          const uploadedFile = (ans.files || []).find(file => file.docId === doc.id);
  ...
  if (missing.length > 0) {
    const labelList = missing.map(m => m.num ? `Q ${m.num}` : m.label || m.id).join(', ');
    return { success: false, error: `Please answer all mandatory questions and upload all required documents: ${labelList}` };
  }
...
export function getAnswersByApplication(appId) {
  return (_db.applicationAnswers||[]).filter(a => a.applicationId === appId);
}

export function getAnswerByField(appId, fieldId) {
  return (_db.applicationAnswers||[]).find(a => a.applicationId === appId && a.fieldId === fieldId);
}

export function saveAnswer(appId, fieldId, value, files = []) {
  let ans = _db.applicationAnswers.find(a => a.applicationId === appId && a.fieldId === fieldId);
  if (ans) {
    ans.value = value;
    ...
    _db.applicationAnswers.push({
      ...
      questionScore: 0,
      adminRemarks: '',
      files: files.map(f => ({ ...f, fileStatus: DOC_STATUS.PENDING, fileRejectionReason: '' })),
      updatedAt: new Date().toISOString()
    });
  }
  scheduleSave();
...
export function submitQuestion(appId, fieldId, userId) {
  ...
  if (field?.mandatory) {
    ...
    if (isUploadType) {
      if (!ans.files || ans.files.length === 0) {
        return { success: false, error: 'At least one supporting document must be uploaded.' };
      }
    } else {
      if (!ans.value) {
        return { success: false, error: 'Answer is required before submitting.' };
      }
    }
  }
  // Check mandatory docs
  ...
  for (const doc of mandatoryDocs) {
    if (!ans.files?.find(f => f.docId === doc.id)) return { success: false, error: `Mandatory document "${doc.name}" must be uploaded.` };
  }
  autoCalculateScore(appId, fieldId);
  ans.questionStatus = QUESTION_STATUS.SUBMITTED;
  ans.updatedAt = new Date().toISOString();
  addTimelineEntry(appId, `Question submitted: ${field?.label || fieldId}`, userId);
  _save();
  return { success: true };
}

// ─── Question-Level Approve / Reject ─────────────────────────
export function approveQuestion(appId, fieldId, adminId, score = null, remarks = '') {
  const ans = _db.applicationAnswers.find(a => a.applicationId === appId && a.fieldId === fieldId);
  if (!ans) return false;
  const field = getFieldById(fieldId);
  
  // Guard: question must be filled before it can be approved
  if (!isQuestionFilled(ans, field)) {
    throw new Error('Cannot approve unanswered question. Question contains no valid response.');
  }
  ...
  if (score !== null) {
    ...
    ans.questionScore = isScorableAnswer(ans, field) ? (field?.maxScore || field?.weight || 1) : 0;
  }
  ...
  const reviewerName = reviewerUser ? (reviewerUser.name || reviewerUser.username) : adminId;
  addTimelineEntry(appId, `Question approved by ${reviewerName}: ${field?.num || ''} ${field?.label || fieldId}`, adminId, remarks);
  _save();
  return true;
}
...
export function rejectQuestion(appId, fieldId, adminId, reason) {
  ...
  const reviewerUser = getUserById(adminId);
  const reviewerName = reviewerUser ? (reviewerUser.name || reviewerUser.username) : adminId;
  addTimelineEntry(appId, `Question rejected by ${reviewerName}: ${field?.num || ''} ${field?.label || fieldId}`, adminId, reason);
  _save();
  return true;
}

export function requestAdditionalDocsForQuestion(appId, fieldId, adminId, remarks) {
  let ans = _db.applicationAnswers.find(a => a.applicationId === appId && a.fieldId === fieldId);
  const field = getFieldById(fieldId);
  if (!ans) {
    ans = {
  ...
  const reviewerUser = getUserById(adminId);
  const reviewerName = reviewerUser ? (reviewerUser.name || reviewerUser.username) : adminId;
  addTimelineEntry(appId, `Additional documents requested by ${reviewerName} for question: ${field?.num || ''} ${field?.label || fieldId}`, adminId, remarks);
  _save();
  return true;
}
...
export function rejectDocument(appId, fieldId, docId, adminId, reason) {
  ...
  addTimelineEntry(appId, `Document rejected: ${file.name} — ${reason}`, adminId);
  _save();
  return true;
}
...
function _recalcScore(appId) {
  ...
  if (idx !== -1) {
    ...
    if (oldScore !== score) {
      ...
      addTimelineEntry(appId, `Score updated: ${score} marks (previously ${oldScore})`, currentUserId);
    }
  }
}
...
export function calculateApplicationProgress(appId) {
  ...
  const completed = fields.filter(f => isQuestionFilled(answersMap[f.id], f)).length;
...
export function getQuestionReviewQueue(editionId, filters = {}) {
  const apps = (_db.applications || []).filter(a => a.editionId === editionId);
  const rows = [];
  apps.forEach(app => {
    const user = getUserById(app.userId);
    ...
    answers.forEach(ans => {
      ...
      if (filters.reformAreaId && row.reformAreaId !== filters.reformAreaId) return;
      rows.push(row);
    });
  });
...
export function getUserById(id) { return (_db.users||[]).find(u => u.id === id); }
export function getUserByUsername(u) { return (_db.users||[]).find(x => x.username === u); }
...
export function getNotifications(userId) {
  return (_db.notifications||[]).filter(n => n.userId === userId && !n.isDismissed).sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
}
export function getUnreadCount(userId) {
  return (_db.notifications||[]).filter(n => n.userId === userId && !n.read && !n.isDismissed).length;
}
export function addNotification(userId, eventType, message, applicationId = null) {
  const now = Date.now();
  const key = `${eventType}_${applicationId || 'none'}_${userId}`;
  
  const dup = (_db.notifications||[]).find(n =>
    (n.notificationKey === key || (n.userId === userId && n.eventType === eventType && n.applicationId === applicationId)) &&
  ...
  const note = {
    id: 'note_' + Date.now() + '_' + Math.random().toString(36).substr(2,5),
    userId, eventType, message, applicationId,
    notificationKey: key,
    read: false, isDismissed: false, createdAt: new Date().toISOString()
  };
  if (!_db.notifications) _db.notifications = [];
  _db.notifications.unshift(note);
  if (_db.notifications.length > 500) _db.notifications = _db.notifications.slice(0, 500);
  scheduleSave();
  return note;
}
...
export function markAllNotificationsRead(userId) {
  (_db.notifications||[]).filter(n => n.userId === userId).forEach(n => { n.read = true; });
  _save();
}
export function dismissNotification(userId, notifId) {
  const n = (_db.notifications||[]).find(n => n.id === notifId && n.userId === userId);
  if (n) {
    n.isDismissed = true;
    _save();
    return true;
...
export function getNotStartedCount(userId) {
  ...
  editions.forEach(e => {
    ...
    const hasAssigned = allSections.some(sec => isSectionAssignedToUser(sec, userId));
    if (hasAssigned) {
...
export function getAuditLogs(filters = {}) {
  ...
  if (filters.userQuery) {
    ...
    const matchingUserIds = new Set(
      (_db.users || [])
        .filter(u => (u.username || '').toLowerCase().startsWith(query) || (u.name || '').toLowerCase().startsWith(query))
        .map(u => u.id)
    );
    logs = logs.filter(l => matchingUserIds.has(l.userId) || (l.userId || '').toLowerCase().startsWith(query));
  ...
  if (filters.district) {
    const districtUserIds = new Set(
      (_db.users || []).filter(u => u.district === filters.district).map(u => u.id)
    );
  ...
  if (filters.category) {
    ...
    if (cat === 'login') {
      logs = logs.filter(l => {
        ...
        return act.includes('portal accessed') || act.includes('login') || act.includes('logout') || act.includes('password') || l.entityType === 'auth';
      });
    } else if (cat === 'approve') {
      logs = logs.filter(l => {
        const act = (l.action || '').toLowerCase();
      ...
      logs = logs.filter(l => {
        const act = (l.action || '').toLowerCase();
        return act.includes('rejected') || act.includes('rejection') || act.includes('resubmission') || act.includes('resubmit') || act.includes('additional docs') || act.includes('additional documents');
      });
    } else if (cat === 'assign') {
      logs = logs.filter(l => {
        const act = (l.action || '').toLowerCase();
        return act.includes('assigned') || act.includes('reassigned') || act.includes('unassigned') || act.includes('assignment') || act.includes('mappings');
      });
    }
  }
  ...
  const page = filters.page || 1, pageSize = filters.pageSize || 100;
  return { items: logs.slice((page-1)*pageSize, page*pageSize), total: logs.length, totalPages: Math.ceil(logs.length/pageSize) };
}
...
export function getAllAssignments() { 
  const activeEditionIds = getEditions(false).map(e => e.id);
  return (_db.assignments || []).filter(a => activeEditionIds.includes(a.editionId)); 
}
export function getAssignments(userId) { 
  const activeEditionIds = getEditions(false).map(e => e.id);
  return (_db.assignments||[]).filter(a => a.userId === userId && activeEditionIds.includes(a.editionId)); 
}
...
export function isFieldAnsweredForStats(ans, field) {
  ...
  if (field.fieldType === 'radio') {
    return ans.value === 'Yes' || ans.value === 'No' || (ans.value && ans.value !== '');
  }
  return typeof ans.value === 'string' && ans.value.trim() !== '';
}

export function getPendingAssignmentsCount(userId, editionId = null) {
  const assigned = getAssignments(userId).filter(a => !editionId || a.editionId === editionId);
  const assignedFieldIds = new Set();
  
  const activeEditions = getEditions(false).map(e => e.id);
  const fields = (_db.formFields || []).filter(f => activeEditions.includes(f.editionId) && !f.isLayoutElement);

  assigned.forEach(a => {
    if (a.type === 'Question') {
      assignedFieldIds.add(a.fieldId || a.questionId);
    } else if (a.type === 'Action Point') {
      fields.filter(f => f.actionPointId === a.actionPointId).forEach(f => assignedFieldIds.add(f.id));
    } else if (a.type === 'Reform Area') {
      fields.filter(f => f.reformAreaId === a.reformAreaId).forEach(f => assignedFieldIds.add(f.id));
    }
  });
...
export function createAssignment(userId, data, assignedBy) {
  ...
  if (exists) {
    const existing = (_db.assignments || []).find(x => 
      x.userId === userId &&
      x.editionId === data.editionId &&
      x.type === data.type &&
      (data.type === 'Reform Area' ? (x.sectionId === data.sectionId || x.reformAreaId === data.reformAreaId) : true) &&
...
export function getEditionStats(editionId) {
  ...
  try {
    ...
    if (sessionRaw) {
      const user = JSON.parse(sessionRaw);
      if (user && (user.role === 'admin' || user.role === 'reviewer' || user.role === 'superadmin')) {
        apps = apps.filter(a => a.status !== 'Draft');
      }
    }
  } catch (e) {}
  const scores = apps.map(a => calculateApplicationScore(a.id)).filter(s => s > 0);
  return {
    total: apps.length,
    draft: apps.filter(a => a.status === 'Draft').length,
    submitted: apps.filter(a => ['Submitted','Resubmitted'].includes(a.status)).length,
    underReview: apps.filter(a => a.status === 'Under Review').length,
    approved: apps.filter(a => a.status === 'Approved').length,
    rejected: apps.filter(a => a.status === 'Rejected').length,
    additionalDocs: apps.filter(a => a.status === 'Additional Documents Requested').length,
  ...
  return reformAreas.map(ra => {
    ...
    apps.forEach(app => {
      ...
      fields.forEach(f => {
        ...
        if (ans?.questionScore) { totalScore += ans.questionScore; count++; }
      });
    });
    return { ...ra, avgScore: count ? Math.round(totalScore/count) : 0, totalScore };
  });
}
...
export function getAnalytics(editionId) {
  ...
  apps.forEach(app => {
    ...
    const key = new Date(app.submittedAt).toLocaleDateString('en-IN', { day:'2-digit', month:'short' });
    if (key in daily) daily[key]++;
  });
  // Score buckets
  const buckets = {'0-10':0,'11-20':0,'21-30':0,'31-40':0,'41-50':0,'51-60':0,'61-70':0,'71-80':0,'81-90':0,'91-100':0};
  apps.forEach(app => {
  ...
  return { daily, buckets, catScores, reformAreaScores, stats: getEditionStats(editionId) };
}
...
export function checkAndConsolidateSRF(editionId, organization) {
  ...
  const allAssignments = getAllAssignments().filter(a => a.editionId === editionId);
  ...
  for (const uid of assignedUserIds) {
    const userApp = (db.applications || []).find(app => app.editionId === editionId && app.userId === uid);
    if (!userApp) return; // Not all assigned users have applications yet
    if (userApp.status !== 'Submitted') return; // Not all have submitted
    apps.push(userApp);
  }
  
  console.log(`[Consolidation] All ${apps.length} assigned users have submitted for ${organization}. Consolidating...`);
  ...
  db.applicationAnswers = (db.applicationAnswers || []).filter(ans => ans.applicationId !== masterAppId);
  ...
  // Merge answers
  apps.forEach(app => {
    const answers = (db.applicationAnswers || []).filter(ans => ans.applicationId === app.id);
    answers.forEach(ans => {
      const masterAns = {
        ...ans,
  ...
  try {
    addAuditLog('system', `Consolidated SRF records for ${organization} into master application ${masterAppId}`, 'application', masterAppId);
  } catch (e) {
...
export function submitApplication(appId, userId) {
  ...
  const hasAssignment = (_db.assignments || []).some(a => a.userId === userId && a.editionId === app.editionId);
  if (!hasAssignment) {
    return { success: false, error: 'No active assignments found for this user in this edition.' };
  }
  
  if (app.status === 'Submitted' || app.status === 'Approved' || app.status === 'Under Review' || app.submittedAt) {
    return { success: false, error: 'Application has already been submitted.' };
  }
  
  const fields = getFieldsByEdition(app.editionId).filter(f => isFieldAssignedToUser(f, userId));
  const answers = getAnswersByApplication(appId);
  const answersMap = {};
  answers.forEach(a => { answersMap[a.fieldId] = a; });
  ...
  const wasRejectedOrRequested = (app.statusHistory || []).some(h => h.status === 'Rejected' || h.status === 'Additional Documents Requested') || app.rejectionReason || app.status === 'Additional Documents Requested';
  ...
  const answersSnapshot = answers.map(ans => ({
    ...
    adminRemarks: ans.adminRemarks,
    files: (ans.files || []).map(f => ({ docId: f.docId, name: f.name, fileStatus: f.fileStatus, fileRejectionReason: f.fileRejectionReason }))
  }));
  ...
  if (!app.assignedReviewer) {
    ...
    if (admins.length > 0) {
      const workloads = {};
      admins.forEach(adm => { workloads[adm.id] = 0; });
      (_db.applications || []).forEach(a => {
        if (a.assignedReviewer && ['Submitted', 'Under Review', 'Resubmitted'].includes(a.status)) {
      ...
      addNotification(leastReviewer, 'APPLICATION_ASSIGNED', `Application ${app.id} has been automatically assigned to you.`, app.id);
    }
  }
  ...
  if (!verifiedApp || verifiedApp.status !== finalStatus || !verifiedApp.reviewQueue || !verifiedApp.visibleToAdmin || !verifiedApp.visibleToSuperAdmin) {
    console.error(`[Submit Verification Error] Verification failed for app: ${appId}`);
    return { success: false, error: 'Database verification failed after submission.' };
  }

  addTimelineEntry(appId, wasRejectedOrRequested ? 'Application resubmitted' : 'Application submitted', userId);
  addAuditLog(userId, wasRejectedOrRequested ? `Resubmitted application: ${appId}` : `Submitted application: ${appId}`, 'application', appId);
...
export function createDepartment(data) {
  _db.departments = _db.departments || [];
  const nameExists = _db.departments.some(d => d.name.toLowerCase() === data.name.toLowerCase());
  const codeExists = _db.departments.some(d => d.code.toLowerCase() === data.code.toLowerCase());
...
export function updateDepartment(id, data) {
  ...
  const nameExists = _db.departments.some(d => d.id !== id && d.name.toLowerCase() === data.name.toLowerCase());
  const codeExists = _db.departments.some(d => d.id !== id && d.code.toLowerCase() === data.code.toLowerCase());
  if (nameExists) return { error: 'Department name already exists.' };
  if (codeExists) return { error: 'Department code already exists.' };
...
export function getMessagesBetween(userIdA, userIdB) {
  return (_db.messages || [])
    .filter(m => (m.senderId === userIdA && m.receiverId === userIdB) || (m.senderId === userIdB && m.receiverId === userIdA))
    .sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
}

export function getUnreadMessageCountFrom(senderId, receiverId) {
  return (_db.messages || [])
    .filter(m => m.senderId === senderId && m.receiverId === receiverId && !m.read).length;
}
...
export function restoreFromRecycleBin(id) {
  ...
  if (item.type === 'edition') {
    ...
    (item.reformAreasData || []).forEach(ra => {
      if (!_db.reformAreas.some(r => r.id === ra.id)) _db.reformAreas.push(ra);
    });
    if (!_db.formFields) _db.formFields = [];
    (item.fieldsData || []).forEach(f => {
      if (!_db.formFields.some(field => field.id === f.id)) _db.formFields.push(f);
    });
    if (!_db.applications) _db.applications = [];
    (item.appsData || []).forEach(app => {
      if (!_db.applications.some(a => a.id === app.id)) _db.applications.push(app);
    });
...
export function assignApplicationReviewer(appId, reviewerId, assignedBy) {
  ...
  if (app) {
    ...
    addNotification(reviewerId, 'APPLICATION_ASSIGNED', `Application ${app.id} has been manually assigned to you.`, app.id);
    _save();
    return true;
  }
  return false;
...
export function removeApplicationReviewer(appId, removedBy) {
  ...
  if (app) {
    ...
    if (oldReviewer) {
      addNotification(oldReviewer, 'APPLICATION_ASSIGNED', `You have been unassigned from Application ${app.id}.`, app.id);
    }
    _save();
    return true;
  }
...
export function autoAssignApplicationReviewer(appId) {
  ...
  admins.forEach(adm => { workloads[adm.id] = 0; });
  (_db.applications || []).forEach(a => {
    if (a.assignedReviewer && a.status !== 'Draft' && a.status !== 'Approved' && a.status !== 'Rejected') {
      if (workloads[a.assignedReviewer] !== undefined) {
        workloads[a.assignedReviewer]++;
      }
    }
...
export function escalateApplicationReview(appId, escalationDetails) {
  ...
  if (app) {
    ...
    addNotification(escalationDetails.assignedTo, 'REVIEW_ESCALATED', `Application ${app.id} has been escalated to you by ${escalationDetails.escalatedBy}.`, app.id);
    _save();
    return true;
  }
  return false;
...
export function validateApplicationAssignmentLink(userId, editionId) {
  ...
  const userAssignments = (_db.assignments || []).filter(a => a.userId === userId && a.editionId === editionId);
  const app = (_db.applications || []).find(a => a.userId === userId && a.editionId === editionId && a.status !== 'Rejected');
  
  if (userAssignments.length > 0 && !app) {
    // Missing application placeholder! Create it.
    console.warn(`[Integrity Repair] Found assignments for user ${userId} on edition ${editionId} but no application draft. Creating application draft.`);
    const user = getUserById(userId);
...
export function runDatabaseIntegrityCheck() {
  ...
  (_db.applications || []).forEach(app => {
    if (!editionsSet.has(app.editionId)) {
      errors.push(`Application ${app.id} references non-existent edition ${app.editionId}`);
    }
    if (!usersSet.has(app.userId)) {
      errors.push(`Application ${app.id} references non-existent user ${app.userId}`);
    }
  });

  (_db.assignments || []).forEach(assign => {
    if (!editionsSet.has(assign.editionId)) {
      errors.push(`Assignment ${assign.id} references non-existent edition ${assign.editionId}`);
    }
    if (!usersSet.has(assign.userId)) {
      errors.push(`Assignment ${assign.id} references non-existent user ${assign.userId}`);
    }
  });

  if (errors.length > 0) {
    console.error(`[Integrity Audit] Violations found:`, errors);
    addAuditLog('system', `Database Integrity Audit failed: ${errors.length} errors found.`, 'integrity_audit', 'system');
  } else {
...
export function repairDataIntegrity() {
  if (!_db) return;
  console.log('[Integrity Repair] Starting database integrity check & repair...');
  ...
  _db.applications = (_db.applications || []).filter(app => {
    ...
    if (!valid) {
      _db.applicationAnswers = (_db.applicationAnswers || []).filter(ans => ans.applicationId !== app.id);
    }
    return valid;
  });
    ...
    activeUsers.forEach(user => {
      ...
      if (!existingApp) {
        const newApp = {
          ...
          additionalDocsNote: '',
          reviewerComments: '',
          timeline: [{ action: 'Application created via system repair', timestamp: new Date().toISOString(), by: 'system_repair' }],
          comments: [],
          reformAreaStatuses: {},
          statusHistory: [{ status: 'Draft', timestamp: new Date().toISOString(), by: 'system_repair' }],
          submissions: []
        };
        _db.applications.push(newApp);
  ...
  (_db.assignments || []).forEach(assign => {
    const existingApp = (_db.applications || []).find(a => a.userId === assign.userId && a.editionId === assign.editionId && a.status !== 'Rejected');
    if (!existingApp) {
      ...
      if (user) {
        const newApp = {
          ...
          additionalDocsNote: '',
          reviewerComments: '',
          timeline: [{ action: 'Application created via system repair (missing assignment link)', timestamp: new Date().toISOString(), by: 'system_repair' }],
          comments: [],
          reformAreaStatuses: {},
          statusHistory: [{ status: 'Draft', timestamp: new Date().toISOString(), by: 'system_repair' }],
          submissions: []
  ...
  if (stats.assignmentsRepaired > 0 || stats.applicationsRepaired > 0 || stats.reviewersAssigned > 0 || stats.scoresCorrected > 0 || stats.answersReset > 0) {
    addAuditLog('system', `Database Integrity Repair executed: ${JSON.stringify(stats)}`, 'integrity', 'system');
    _save();
  }
...
function recalculateExistingApplications() {
  ...
  (_db.applicationAnswers || []).forEach(ans => {
    ...
    if (ans.questionScore > 0 && !isScorableAnswer(ans, field)) {
      console.warn('[Store] Repair: zeroing phantom score for answer', ans.fieldId, 'value:', ans.value);
      ans.questionScore = 0;
      changed = true;
    }
  });
